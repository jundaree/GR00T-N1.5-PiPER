...
RUN apt-get update && \
    ...
    apt-get update && \
    \
    # 2. Install FFmpeg, the H.264 encoder (libx264-dev), and display libraries
    apt-get install -y ffmpeg libsm6 libxext6 libx264-dev && \
    \
    # 3. Clean up package lists and archives to minimize final image size
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*


# Install NumPy first, pinned to a version compatible with TensorFlow/gr00t (<2.0.0).
# Then install the other Python libraries needed for your script.
RUN pip install --no-cache-dir \
    "numpy<2.0.0" \
    pandas \
    pyarrow \
    ffmpeg-python \
    opencv-contrib-python==4.9.0.80 # Pinned to avoid NumPy 2.x conflict
